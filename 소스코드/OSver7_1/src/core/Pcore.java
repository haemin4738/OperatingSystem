...
public class Pcore extends Core{
	...
	public void setComponent(int time) {
		...
		if (this.getProcess().getBurstTime() % 2 == 0)
			this.getProcess().setWaitingTime(this.getProcess().getturnaroundTime() - this.getProcess().getBurstTime() / 2);
         else
            this.getProcess().setWaitingTime(this.getProcess().getturnaroundTime() - (this.getProcess().getBurstTime() + 1) / 2);
	}
